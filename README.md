...
## Synzi Notes on 3rd Party Libraries with Native Functionality

Sometimes it is necessary to link 3rd party libraries as we extend this codebase's functionality. Because of how the JitsiMeet app is constructed, an SDK (jitsimeet) is compiled first and then the app targets are created with a dependency to it. Running ```react native link``` often does not work as intended as it's linking against the app targets themselves and not the SDK. When adding [react-native-device-info](https://github.com/rebeccahughes/react-native-device-info) for example, I had to add the native module to the SDK itself in Android. For example, navigate to the Java class 'ReactInstanceManagerHolder.java' in the SDK and import any new native module that came with the package. Please see 'RNDeviceModule' as a reference. In iOS, ```react native link``` seems to have no effect whatsover. The only way to expose native modules there is via CocoaPods. Adding a reference to the library in the pod file might look like this. ```pod 'RNDeviceInfo', :path => '../node_modules/react-native-device-info'``` Please re-run 'pod install' for changes to take effect. 

## Launch Care Connect app via the command line
```react-native run-ios --scheme Care-Connect``` 
