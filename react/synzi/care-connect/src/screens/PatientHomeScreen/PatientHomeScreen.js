...
export default class PatientHomeScreen extends Component {
  ...
  async componentDidMount() {
    ...
    const mounted = true
    this.setState({ userId, username, displayName, mounted, showMessages: messagePermissions })    
    
    // Sets the logout function for use in react navigation on this page
    this.props.navigation.setParams({ displayName: displayName, logout: this.logout, handleEnvPicker: this.handleEnvPicker, showDrawer: messagePermissions })
  ...
  render() {
    ...
    const { userId, mounted, showMessages } = this.state

    StatusBar.setBarStyle('dark-content', true)
    StatusBar.setBackgroundColor('white')

    Reactotron.log("Patient HomeScreen render with userid = ", userId, " :: mounted? ", mounted)
                ...
                if(data && data.user){
                    ...
                    return (
                      <View style={styles.outerContainer}>
                        ...
                        <View style={styles.mainContainerStyle}>
                          <ScrollView>
                            <CareTeamView selectedPatientUserId={userId} makeCall={makeCall} showMessages={showMessages} />
                            <GroupsView userId={userId} makeGroupCall={makeGroupCall} />
